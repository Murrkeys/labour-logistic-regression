...
Read the manual page for the `?` dataset for background information.
...
Now fit a series of logistic regression models to the data. The first model should use `RET` as the outcome and the other four variables as predictors. Then fit additional models, each with a single pair-wise interaction term added --- so the first additional model would have the four predictors plus an interaction terms between `SM` and `DIAB`, the next model would have the four predictors plus an interaction terms between `SM` and `GH`, and so on. Make sure you have fitted models with all possible pair-wise interaction terms. Thus, each additional model should have the four predictors plus a single interaction term.
...
Use `unclass()` on each of the three predictor variables in task 3.b above to convert them to numeric variables and re-fit the model. Can the model be simplified? Justify your response? (2 marks)
